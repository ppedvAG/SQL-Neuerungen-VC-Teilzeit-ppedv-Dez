...
select customerid, employeeid, orderdate, freight, shipcity into Best from orders

insert into best
select * from best
GO 8

update best set freight = freight * RAND(convert(varbinary, newid()))*100
select freight, count(*) from best group by freight

select * from best where dbo.fbrutto(freight,1.19) < 2
